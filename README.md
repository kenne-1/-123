...
项目支持通过 OpenAI API 对选中的热点生成语义解释、商业价值、内容建议、风险提示和 CTA。
...
7. AI Prompt：根据当前候选词条和判断结果，生成结构化的大模型分析指令。
8. 大模型调用：配置 API Key 后，可在页面中点击按钮直接获得内容策略建议；未配置时仍可复制 Prompt 手动使用。
9. 报告导出：支持导出候选结果 CSV 和 Markdown 报告。
